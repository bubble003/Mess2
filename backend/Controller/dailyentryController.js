...
export const updateDailyEntry = asyncHandler(async (req, res) => {
  ...
  if (verifyThing === "breakfast") {
    updatedObject = {
      breakfast: true,
      lunch: length == 0 ? false : isTodayAdded[0].menu.lunch,
      dinner: length == 0 ? false : isTodayAdded[0].menu.dinner,
    };
  } else if (verifyThing === "lunch") {
    updatedObject = {
      breakfast: length == 0 ? false : isTodayAdded[0].menu.breakfast,
      lunch: true,
      dinner: length == 0 ? false : isTodayAdded[0].menu.dinner,
    };
  ...
  if (isTodayAdded.length === 1) {
    if (
      (verifyThing === "breakfast" && isTodayAdded[0].menu.breakfast) ||
      (verifyThing === "lunch" && isTodayAdded[0].menu.lunch) ||
      (verifyThing === "dinner" && isTodayAdded[0].menu.dinner)
    ) {
      // console.log("here is problem");
      return res
        .status(400)
        .json({ message: `Your ${verifyThing} entry is already added` });
    }
